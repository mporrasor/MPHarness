...
2. **Living Documentation (States & Tracking):** All state documentation (`TASKS.md`, `MANUAL.md`, `TRACKING.md`, `SPECIFICATION.md`) is a living entity. You MUST keep it strictly updated in real-time.
   ...
   - **Auto-Maintenance (Scale):** You MUST proactively monitor the size of these documents. If `SPECIFICATION.md` becomes too large, autonomously modularize it by extracting domains into a `/specs/` directory and linking them. If `TASKS.md` accumulates too many completed items, autonomously move finished phases to an `/archive/TASKS_archive.md` file to keep the active context clean and token-efficient. Do this without waiting for user prompts.
